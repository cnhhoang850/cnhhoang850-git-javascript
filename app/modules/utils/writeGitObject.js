...
function writeGitObject(hash, content) {
  ...
  if (fs.existsSync(path.join(".git", "objects", objectFolder))) {
    throw new Error("Folder already exist");
  } else if (fs.existsSync(path.join(".git", "objects", objectName))) {
    throw new Error("Git object already exist");
  }

  fs.mkdirSync(path.join(".git", "objects", objectFolder), {
    recursive: true,
  });

  fs.writeFileSync(
    path.join(".git", "objects", objectFolder, objectName),
